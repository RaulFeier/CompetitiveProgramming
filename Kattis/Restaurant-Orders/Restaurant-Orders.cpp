...
void solve() {
  ...
  while (M--) {
    ...
    vector<vector<int>> dp(N + 1, vector<int>(e + 1, 1e9));
    vector<int> freq(e + 1, 0);

    for (int i = 1; i <= N; i++) {
      for (int j = e; j >= v[i - 1]; j--) {
        dp[i][j] = min(dp[i - 1][j], dp[i - 1][j - v[i - 1]] + ++freq[j]);
      }
    }
  }
}
